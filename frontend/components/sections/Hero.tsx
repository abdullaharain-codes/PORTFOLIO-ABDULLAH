'use client'
import { useTypewriter } from '@/lib/hooks/useTypewriter'
import { personal } from '@/lib/data/portfolio'
import { cn } from '@/lib/utils/helpers'

export default function Hero() {
  const typed = useTypewriter(personal.rotatingTitles)

  const scrollTo = (id: string) => {
    document.getElementById(id)?.scrollIntoView({ behavior: 'smooth' })
  }

  return (
    <section
      id="hero"
      className="relative min-h-screen flex items-center pt-24 pb-16 overflow-hidden"
    >
      {/* Atmospheric background */}
      <div className="absolute inset-0 pointer-events-none">
        <div
          className="absolute top-[-100px] right-[-100px] w-[600px] h-[600px] rounded-full"
          style={{ background: 'radial-gradient(circle,rgba(139,92,246,0.07) 0%,transparent 70%)', filter: 'blur(60px)' }}
        />
        <div
          className="absolute bottom-0 left-[-150px] w-[500px] h-[500px] rounded-full"
          style={{ background: 'radial-gradient(circle,rgba(139,92,246,0.04) 0%,transparent 70%)', filter: 'blur(60px)' }}
        />
      </div>

      <div className="section-wrapper w-full relative z-10">
        <div className="max-w-[820px]">

          {/* Available badge */}
          <div
            className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full mb-8 border"
            style={{
              background: 'rgba(139,92,246,0.08)',
              borderColor: 'rgba(139,92,246,0.22)',
              animation: 'fadeUp 0.7s ease 0.1s both',
            }}
          >
            <span
              className="w-1.5 h-1.5 rounded-full bg-accent animate-pulse-dot"
              style={{ boxShadow: '0 0 6px rgba(139,92,246,0.8)' }}
            />
            <span className="font-mono text-[11px] font-semibold text-accent tracking-[0.06em] uppercase">
              Available for Work
            </span>
          </div>

          {/* Name */}
          <h1
            className="font-display font-extrabold tracking-tight leading-[1.05] mb-3 text-gradient"
            style={{
              fontSize: 'clamp(40px,6vw,76px)',
              animation: 'fadeUp 0.7s ease 0.2s both',
            }}
          >
            {personal.name}
          </h1>

          {/* Typewriter title */}
          <div
            className="flex items-center gap-3 mb-6 flex-wrap"
            style={{
              fontSize: 'clamp(18px,2.8vw,34px)',
              animation: 'fadeUp 0.7s ease 0.3s both',
            }}
          >
            <span className="font-display font-semibold text-text-muted">I&apos;m a</span>
            <span className="font-display font-semibold text-accent">
              {typed}
              <span className="animate-blink border-r-2 border-accent ml-0.5" />
            </span>
          </div>

          {/* Short bio */}
          <p
            className="text-text-secondary text-[15px] leading-relaxed max-w-[560px] mb-10"
            style={{ animation: 'fadeUp 0.7s ease 0.4s both' }}
          >
            {personal.shortBio}
          </p>

          {/* CTA buttons */}
          <div
            className="flex flex-wrap gap-3 mb-14"
            style={{ animation: 'fadeUp 0.7s ease 0.5s both' }}
          >
            <a href={personal.resumeUrl} download className="btn-primary">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
              Download Resume
            </a>
            <a href={personal.socials.github} target="_blank" rel="noopener noreferrer" className="btn-secondary">
              <GitHubIcon />
              GitHub
            </a>
            <a href={personal.socials.linkedin} target="_blank" rel="noopener noreferrer" className="btn-secondary">
              <LinkedInIcon />
              LinkedIn
            </a>
          </div>

          {/* Stats */}
          <div
            className="flex gap-8 md:gap-10 flex-wrap"
            style={{ animation: 'fadeUp 0.7s ease 0.6s both' }}
          >
            {[
              { num: personal.stats.yearsLearning,    label: 'Years Learning' },
              { num: personal.stats.projectsBuilt,    label: 'Projects Built' },
              { num: personal.stats.technologiesUsed, label: 'Technologies' },
            ].map((stat, i) => (
              <div key={i} className="flex items-center gap-6">
                {i > 0 && <div className="w-px h-10 bg-border-default self-stretch" />}
                <div className="flex flex-col gap-1">
                  <span className="font-display font-bold text-[28px] text-white leading-none">
                    {stat.num}<span className="text-accent">+</span>
                  </span>
                  <span className="font-mono text-[10px] text-text-muted uppercase tracking-[0.08em]">
                    {stat.label}
                  </span>
                </div>
              </div>
            ))}
          </div>
        </div>
      </div>

      {/* Scroll indicator */}
      <div
        className="absolute bottom-10 left-10 md:left-20 flex items-center gap-3"
        style={{ animation: 'fadeUp 0.7s ease 0.8s both' }}
      >
        <div
          className="relative overflow-hidden"
          style={{ width: 40, height: 1, background: '#2A2A2A' }}
        >
          <div className="absolute inset-y-0 w-full bg-accent animate-[scrollLine_2s_ease-in-out_2s_infinite]" />
        </div>
        <span className="font-mono text-[10px] text-text-muted tracking-[0.12em] uppercase">
          scroll to explore
        </span>
      </div>

      <style>{`
        @keyframes fadeUp {
          from { opacity:0; transform:translateY(18px) }
          to   { opacity:1; transform:translateY(0) }
        }
        @keyframes scrollLine {
          0%   { left:-100% }
          50%  { left:0% }
          100% { left:100% }
        }
      `}</style>
    </section>
  )
}

function GitHubIcon() {
  return (
    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
      <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
    </svg>
  )
}

function LinkedInIcon() {
  return (
    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
    </svg>
  )
}
